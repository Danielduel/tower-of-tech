(
  async () => {
    ...
    const fetchPage = async (playerId, page = 1, itemsPerPage = 100) =>
      (await (await fetch(`https://scoresaber.com/api/player/${playerId}/scores?limit=${itemsPerPage}&page=${page}`))
        .json())
        ?.playerScores?.map((s) => ({
        ...
        .filter((s) => s.pp); //.filter(s => [293604, 273126].includes(s.leaderboardId)) // uncomment to filter by map
    ...
    try {
      ...
      const csv = "Id,Song,Mapper,Difficulty,PP,Weight,Acc\n" +
        scores.map((s) =>
          `${s.leaderboardId},"${s.songName.replaceAll('"', '""')}","${
            s.mapper.replaceAll('"', '""')
          }",${s.difficulty},${s.pp},${s.weight},${s.acc}`
        ).join("\n");
